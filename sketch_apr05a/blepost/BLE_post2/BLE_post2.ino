...
// Here the json file is initialized. It has several preset values that will be replaced as we scan the bluetooth networks.
...
class MyAdvertisedDeviceCallbacks: public BLEAdvertisedDeviceCallbacks {
    ...
    void onResult(BLEAdvertisedDevice advertisedDevice) {
      ...
      dataGrama += "{\"bssid\": \"bssid(i)\",      \"rssi\": \"rssi(i)\",      \"ssid\": \"ssid(i)\"}";
      Serial.print(number);
      Serial.print(") ");
      ...
      dataGrama.replace("bssid(i)", (advertisedDevice.getAddress().toString().c_str()));
...
void loop() {
  ...
  if ((millis() - lastTime) > timerDelay) {
    ...
    if (WiFi.status() == WL_CONNECTED) {
      ...
      int lastComma = dataGrama.lastIndexOf(',');
      dataGrama.remove(lastComma , 1);
  ...
  //It is necessary to reset the datagram in order to add the new scans
  int lastChar = dataGrama.indexOf("BLEData");
  dataGrama.remove(lastChar + 9);
