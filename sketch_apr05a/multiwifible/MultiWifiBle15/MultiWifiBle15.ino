...
const char* serverNameC = "http://ils.dsi.uminho.pt/ar-ware/S02/i2a/i2aSamples.php";
const char* tagNameC = "tagConstant";
const char* WIFIintervalC = "4000";
const char* BLEintervalC = "9000";
const char* jsonURLC = "http://192.168.1.3/";
...
void getInfo() {
  ...
  if ((WiFi.status() == WL_CONNECTED)) { //Check the current connection status
    ...
    http.begin(jsonURL); //Specify the URL (https://serverfault.com/questions/209150/how-to-access-localhost-by-ip-address#:~:text=To%20access%20the%20server%20from,)%20by%20running%20hostname%20%2DI%20.)
    ...
    if (httpCode > 0) { //Check for the returning code
      ...
      indice12 = (indice.lastIndexOf("jsonURL") - 5);

      indice13 = (indice.lastIndexOf("jsonURL") + 11);
      indice14 = (indice.lastIndexOf("}") - 3);
      ...
      jsonURLGot = payload.substring(indice13, indice14);
      debugln(jsonURLGot);
    }
...
void setup() {
  ...
  if (ssid == "" || password == "" || serverName == "" || tagName == "" || wifiInterval == "" || bleInterval == "" || jsonURL == "") {
    connectToNetwork();
  }
  else {
    // Connect to Wi-Fi
  ...
  preferences.putString("jsonURL", jsonURLGot);
  ...
  if (ssid == "" || password == "" || serverName == "" || tagName == "" || wifiInterval == "" || bleInterval == "" || jsonURL == "") {
    preferences.begin("credentials", false);
    preferences.putString("ssid", ssidC);
    preferences.putString("password", passwordC);
    preferences.putString("serverName", serverNameC);
    preferences.putString("tagName", tagNameC);
    preferences.putString("WIFIinterval", WIFIintervalC);
    preferences.putString("BLEinterval", BLEintervalC);
    preferences.putString("jsonURL", jsonURLC);

    debugln("Credentials Saved using Constant Settings");
